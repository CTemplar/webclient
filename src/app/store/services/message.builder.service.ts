...
export class MessageBuilderService {
  ...
  async getMimeData(
    ...
    if (isPGPMessage) {
      ...
      // Content Body
      const content = isHtml ? (isBase64EncodeForBody ? btoa(mailData.content) : mailData.content) : (isBase64EncodeForBody ? btoa(mailData.content_plain) : mailData.content_plain) || '';
